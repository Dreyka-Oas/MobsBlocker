package oas_work.mobs_blocker.command;
...
public class SpawnblockerCommand {
    ...
    // Méthode pour contourner l'accès privé à la langue en 1.20.1
    private static String getPlayerLanguage(ServerPlayer player) {
        try {
            // Dans les mappings Mojang (utilisés par MCreator), le champ s'appelle "language"
            Field field = ServerPlayer.class.getDeclaredField("language");
            field.setAccessible(true);
            return (String) field.get(player);
        } catch (Exception e) {
            return "en_us"; // Fallback si échec
        }
    }
